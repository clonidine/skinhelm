...
const CAPE_STOPPED_REST_RADIANS: f32 = 0.0;
const CAPE_MOTION_REST_RADIANS: f32 = std::f32::consts::PI * 0.06;
...
pub fn cape_rotation_x(motion: CapeMotion, t: f32) -> f32 {
    match motion {
        CapeMotion::Stopped => CAPE_STOPPED_REST_RADIANS,
        CapeMotion::Idle => t.sin() * 0.01 + CAPE_MOTION_REST_RADIANS,
        CapeMotion::Walking => (t / 1.5).sin() * 0.06 + CAPE_MOTION_REST_RADIANS,
...
mod tests {
    ...
    fn pose_at_phase_zero() {
        ...
        assert!((pose.cape_x - CAPE_MOTION_REST_RADIANS).abs() < 0.0001);
    }

    #[test]
    fn static_pose_keeps_cape_flat_against_back() {
        let pose = static_pose();

        assert_eq!(pose.cape_x, CAPE_STOPPED_REST_RADIANS);
    }
    ...
    fn idle_cape_rotation_is_subtle() {
        ...
        assert!((rotation - (CAPE_MOTION_REST_RADIANS + 0.01)).abs() < 0.0001);
    }
    ...
    fn walking_cape_rotation_uses_slow_swing() {
        ...
        assert!((rotation - (CAPE_MOTION_REST_RADIANS + 0.06)).abs() < 0.0001);
    }
